...
public class Local {
   ...
   private ArrayList tempList=new ArrayList();
    ...
    public Local(String fnam, int nbrItiration, int width, int height, double minBound,double maxBound, double[][] pp){
    ...
    public void local(){
        ...
        for(int k=0;k<nbrItiration;k++){
            ...
            for(int i=0; i<heightsize;i++){
                for(int j=0; j<widthsize;j++){
                    pop[i][j]=pop[i][j]+(-1+(1 - (-1))*rand.nextDouble());
                    if(pop[i][j]<minBound)pop[i][j]=minBound;
                    else if(pop[i][j]>maxBound)pop[i][j]=maxBound;
                }
